...
Views._userline = function(user) {
    var icon = user.away?'user-icon-away':'user-icon-active';
    return '<div class="user" data-nick="'+user.nick+'">'+
    '<span class="user-icon '+icon+'"></span>'+
    '<span class="user-name">'+Views.utils.escapetags(user.nick)+'</span></div>';
};
...
Views.addUser = function(buffer, user) {
    var userline = Views._userline(user);
